...
import optuna
from study_snapshot import load_study_list as load_study_snapshot_list


def load_study_list(result_dir: Path, label: str) -> list[optuna.Study]:
    path = result_dir / f"{label}.pickle"
    study_list = load_study_snapshot_list(path)
    if not isinstance(study_list, list) or any(not isinstance(study, optuna.Study) for study in study_list):
